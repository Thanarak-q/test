...
| หน้า admin สำหรับจัดการ model                             | ยังไม่ทำ            | ตอนนี้มีแค่ API `/v1/admin/models/enable` และ `/disable`                                       |
| checkbox ยอมรับเรื่องส่งข้อมูลให้บุคคลที่สาม ตอนสร้าง key | ยังไม่ทำ            | spec ระบุว่าเป็นเรื่อง UX ไม่ใช่ security control ยังไม่มีใน modal                             |
...
- [ ] ตัดสินว่าจะทำ `/v1/embeddings`, `/v1/models` และหน้า admin หรือไม่ (หัวข้อ 3)
